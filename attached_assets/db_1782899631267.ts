...
export async function deductCredits(data: {
  ...
  try {
    ...
    if (selectResult.rowCount === 0) {
      // No balance row yet for this user — create one with default Free plan
      // values (50 credits) so credit tracking works even before a real
      // signup/auth flow exists. ON CONFLICT keeps this safe under races.
      await client.query(
        `INSERT INTO user_credits (user_id, plan_tier, credits_balance, monthly_quota, extra_credits_balance)
         VALUES ($1, 'free', 50, 50, 0)
...
/**
 * Returns a user's current credit balance and plan tier, creating a default
 * Free-plan row on first read if one doesn't exist yet. Returns null only
 * when the database itself is unavailable (e.g. DATABASE_URL unset).
 */
export async function getUserCredits(userId: string): Promise<{
  ...
  const result = await pool.query(
    `SELECT plan_tier, credits_balance, extra_credits_balance, monthly_quota, credits_used_this_month
  ...
  return {
    ...
    totalBalance: row.credits_balance + row.extra_credits_balance,
    monthlyQuota: row.monthly_quota,
    creditsUsedThisMonth: row.credits_used_this_month,
  };
}
