...
import { createCheckoutSession, fetchPlans, type BillingPlan } from "@/lib/api";

const TEAL = "#0D9488";

// Static display data — price + features shown while live plans load
const PLAN_DISPLAY = [
  {
    id: "free",
    name: "Free Trial",
    ...
    locked: ["Strategy Agent", "Design Agent", "Consensus Agent"],
    cta: "Current Plan",
    ctaDisabled: true,
...
type PlanDisplay = typeof PLAN_DISPLAY[number];
...
export function UpgradeModal({ visible, onClose }: Props) {
  ...
  const [plans, setPlans] = useState<PlanDisplay[]>(PLAN_DISPLAY);
  const [loadingPlanId, setLoadingPlanId] = useState<string | null>(null);

  // Merge Stripe price IDs into display plans when modal opens
  useEffect(() => {
    if (!visible) return;
    fetchPlans().then((livePlans: BillingPlan[]) => {
      if (!livePlans.length) return;
      setPlans((prev) =>
        prev.map((p) => {
          const match = livePlans.find(
            (lp) =>
              lp.name.toLowerCase().includes(p.id) ||
              p.name.toLowerCase().includes(lp.name.toLowerCase()),
          );
          return match ? { ...p, stripePriceId: match.priceId } : p;
        })
      );
    });
  }, [visible]);

  async function handleUpgrade(plan: PlanDisplay) {
    if (plan.ctaDisabled) return;
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);

    if (!plan.stripePriceId) {
      // Stripe not connected — inform user how to enable it
      Alert.alert(
        `Upgrade to ${plan.name}`,
        `${plan.price}${plan.period} · ${plan.credits}\n\nStripe is not connected yet. Ask the app owner to connect Stripe via the Integrations panel in Replit.`,
        [{ text: "OK" }],
      );
      return;
    }
    ...
    try {
      ...
      if (url) {
        ...
        Alert.alert("Error", "Could not start checkout. Please try again later.");
      }
    } catch {
      ...
      <TouchableOpacity
        style={[
          styles.ctaBtn,
          {
            ...
            opacity: plan.ctaDisabled || loading ? 0.7 : 1,
          },
        ]}
        onPress={onUpgrade}
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 20, fontWeight: "700", letterSpacing: -0.4, marginBottom: 3 },
  subtitle: { fontSize: 13, lineHeight: 18 },
  closeBtn: {
    width: 36, height: 36, borderRadius: 18, borderWidth: 1,
    alignItems: "center", justifyContent: "center", marginTop: 2,
  },
  plansRow: { flexDirection: "row", gap: 12, paddingRight: 16 },
  card: { borderRadius: 20, padding: 16, gap: 0 },
  badge: { alignSelf: "flex-start", paddingHorizontal: 9, paddingVertical: 3, borderRadius: 20, marginBottom: 10 },
  badgeText: { fontSize: 10, fontWeight: "700", color: "#fff", letterSpacing: 0.3 },
  badgePlaceholder: { height: 22, marginBottom: 10 },
  ...
  currentText: { fontSize: 10, fontWeight: "600" },
  priceRow: { flexDirection: "row", alignItems: "baseline", gap: 1, marginBottom: 2 },
  price: { fontSize: 28, fontWeight: "800", letterSpacing: -1 },
  period: { fontSize: 14 },
  yearly: { fontSize: 11, fontWeight: "500", marginBottom: 10 },
  creditChip: {
    flexDirection: "row", alignItems: "center", gap: 5,
    paddingHorizontal: 9, paddingVertical: 5, borderRadius: 10, borderWidth: 1,
    marginTop: 8, marginBottom: 12, flexWrap: "wrap",
  },
  creditText: { fontSize: 12, fontWeight: "700" },
  creditSub: { fontSize: 10, flex: 1 },
  divider: { height: StyleSheet.hairlineWidth, marginBottom: 12 },
  featureList: { gap: 7, marginBottom: 16, flex: 1 },
  featureRow: { flexDirection: "row", alignItems: "center", gap: 7 },
  featureText: { fontSize: 12, flex: 1, lineHeight: 16 },
  ctaBtn: {
    flexDirection: "row", alignItems: "center", justifyContent: "center",
    paddingVertical: 11, borderRadius: 14, marginTop: 4,
  },
  ctaText: { fontSize: 13, fontWeight: "700", letterSpacing: -0.1 },
  footerNote: { fontSize: 11, textAlign: "center", marginTop: 14, lineHeight: 16 },
});
